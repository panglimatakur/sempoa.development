...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if(!empty($direction) && $direction == "periode"){
		$periode 		= isset($_POST['periode']) 		? $_POST['periode'] : "";
    	if($periode == "harian"){
	?>
    	<select name="bln" id="bln" style="width:110px; margin:5px 0 9px 9px">
        	<?php
			$t2 = 0;
			while($t2<12){
			$t2++;
			...
			?>
        	<option value='<?php echo $t2; ?>' <?php if((!empty($bln) && $t2 == $bln) || ($t2 == date('m'))){?>selected<?php } ?>>
    	...
    	<select name="thn" id="thn" style="width:90px; margin:5px 0 9px 9px">
        	<?php
			$t3 = date('Y')-70;
			while($t3<date('Y')){
			$t3++;
	...
	?>
        ...
        &nbsp;S/D 
		<select name="thn2" id="thn2" style="width:90px; margin:5px 0 9px 9px">
        	<?php
			$t4 = date('Y')-70;
			while($t4<date('Y')){
			$t4++;
