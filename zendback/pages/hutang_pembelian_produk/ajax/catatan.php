...
?>
<input type='hidden' id='hutang_config' value='"id_facture":"<?php echo @$id_facture; ?>","id_product_buy":"<?php echo $no; ?>"'/>
<div>
    <div class="ibox float-e-margins">
        <div class="ibox-title">
            <h5>Hutang Pembelian Produk / Bahan</h4><br />
        </div>
        <div class="ibox-content">
            ...
            <thead>
               <tr>
                ...
                <th width="19%">No PO</th>
                <th width="18%" style="text-align:center"><b>Total Pembelian</b></th>
                ...
                <tbody>
                  <?php 
                      ...
                      ?>
                      <tr>
                        <td><?php if(!empty($no_faktur)){?><?php echo @$no_faktur; ?> <?php } ?></td>
                        <td>
                        <?php if(!empty($no_po)){?>
                           <?php echo @$no_po; ?>
                        <?php } ?>
                        ...
                        <td style="text-align:center">
							<span id="sisa_bayar_label"><?php echo money("Rp.",$sisa); ?></span>
                        	<input type="hidden" id="sisa_bayar" value="<?php echo $sisa; ?>"/>
                        </td>
                    </tr>
                      <tr>
                        <td colspan="5">
                        ...
                        <table width="100%" border="0" class='form_input'>
                          ...
                          <tbody>
                          <?php
						  $q_hutang = $db->query("SELECT * FROM ".$tpref."cash_debt_credit WHERE ID_CASH_FLOW='".$dt_produk->ID_CASH_FLOW."' AND PLUS_MINUS = '-' ORDER BY ORDINAL ASC");
						  $pembayaran_2 = 0;
						  while($dt_hutang = $db->fetchNextObject($q_hutang)){
							  $pembayaran_2++;
							  $tgl_tempo = "";
							  @$tgl_tempo = $db->first("REMINDER_DATE",$tpref."debt_credit_reminder","WHERE ID_CASH_FLOW='".$dt_produk->ID_CASH_FLOW."' AND DEBT_CREDIT = '1' AND ORDINAL = '".$dt_hutang->ORDINAL."'");
							 
							  @$next_ordinal = $dt_hutang->ORDINAL+1;
							  @$next_tgl_tempo = $db->fob("REMINDER_DATE",$tpref."debt_credit_reminder","WHERE ID_CASH_FLOW='".$dt_produk->ID_CASH_FLOW."' AND DEBT_CREDIT = '1' AND ORDINAL = '".$next_ordinal."'");
						  ?>
                          <tr>
