...
$(document).ready(function(){
	...
	$("#coin_referal_btn").on("click",function(){
		$("#referal").toggle();
	})
		...
		$.ajax({
			...
			success: function(result){
				...
				if(result.io == 1){ 
					...
					setCookie("sidtoken",result.sidtoken);
					...
					$("#login_modal .modal-title").html(result.title)
					$("#login_modal .modal-body").html(result.msg)
			}
			},
            error: function (xhr, status, errorThrown) {
                //The message added to Response object in Controller can be retrieved as following.
...
function activate(){
	coin_number 	= $("#activated").val();
	
	id_coin	 		= $("#id_merchant").val();
	username 		= $("#username").val();
	$("#activate_loader").html("<div style='margin:10px 0  10px 0; color:#993300'><b>Mohon Tunggu Sebentar...</b></div>");
	$.ajax({
		url : apiDir+"/login/model.php",
		type :"POST",
		dataType: "jsonp",
		...
		data 	: {"direction":"activate",
					...
					"sempoakey":"99765"},
		success: function(response){
			$("#activate_loader").html(response.msg);
			if(response.io == "3"){ $(".activate-input").remove(); }
		}
	})
}
