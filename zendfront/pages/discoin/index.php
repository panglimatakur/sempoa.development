...
$discoin_page	= isset($_REQUEST['dpage']) 	? $_REQUEST['dpage']	: "home";
...
if(!empty($file_name)){
	...
	@$phone_merchant 	= $dt_merchant->CLIENT_PHONE;
	...
	@$app_package		= $dt_merchant->CLIENT_PACKAGE;
	@$playstore			= $dt_merchant->PLAY_STORE;
	@$label_merchant	= $dt_merchant->CLIENT_LOGO_LABEL;
	...
	$q_discount_client 	 = $db->query("SELECT * FROM ".$tpref."clients_discounts WHERE ID_CLIENT  = '".$id_coin."'"); 
	$num_discount_client = $db->numRows($q_discount_client); 
}
...
if($user_os == "BlackBerry"){
	...
	include $call->inc("zendfront/templates/discoin","index.php");
