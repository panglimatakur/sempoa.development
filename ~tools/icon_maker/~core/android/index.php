<?php
	...
	if(!is_dir($new_project."/Android/src/com/".$app."/coin"))	{ mkdir($new_project."/Android/src/com/".$app."/coin",0777); 		}
	...
	$type 	   = $info['extension'];
	move_uploaded_file($_FILES['icon']["tmp_name"],"ic_launcher.".$type);
	$folder	   = array();
	$size	   = array();
	$folder[1] = "drawable-hdpi";
	$size[1]   = "72";
	$folder[2] = "drawable-mdpi";
	$size[2]   = "48";
	$folder[3] = "drawable-xhdpi";
	$size[3]   = "96";
	$folder[4] = "drawable-xxhdpi";
	$size[4]   = "144";
	$t = 0;
		...
		while($t<5){
			$t++;
				...
				resizeupload("ic_launcher.".$type,$android_res_folder.$folder[$t],$size[$t]);
		}
